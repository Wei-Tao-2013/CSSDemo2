...
;(function ( $, win, doc, undefined ){
  ...
  "use strict";
	...
	iSelect.elements = {
		...
		//phone number
		phone_us				: $("#phone-us,.operational .contact .phone-us")
	};
	...
	//iselect opening hour service
	win.ohcf = win.ohcf || {};

	ohcf.defaults = {
		ajaxEndPoint 			: "//www.iselect.com.au/iselect-core/opening-hours/api.php", // Needs to be updated to match domain due to CORS errors
		dataType				: "json",
		openingTimesContainer	: $("#openingTimesContainer"),
		openTimeToday 			: $("#openTimeToday"),
		closeTimeToday			: $("#closeTimeToday"),
		week					: $("#week"), // For demostration purposes only
		timezone 				: $("#timezone")
	};

	ohcf.errors = {
		error_404				: "404 - Opening Hours Service not found",
		error_500 				: "500 - Opening Hours Service server error",
		error_503				: "503 - Opening Hours Service service unavailable",
		error_unsuccessful		: "Opening Hours data status unsuccessful",
		error_noCallback		: "'dataOnly' parameter set to 'true' but no callback function supplied for Opening Hours Service function call",
		error_dataOnly			: "'dataOnly' parameter set to 'false' but a callback function has been supplied for Opening Hours Service function call",
	};

	ohcf.openingHoursService = function(vertical, dataset, dataOnly, callback){		
		try{			
			$.ajax({
				url 		: ohcf.defaults.ajaxEndPoint,
				dataType 	: ohcf.defaults.dataType,
				data 		: "vertical=" + vertical + "&dataset=" + dataset,
				cache 		: false,
				statusCode	: {
	    			404 : function(e){
	    				console.error(ohcf.errors.error_404 + ":" + "Error: " + e);
	    			},
	    			500 : function(e){
	    				console.error(ohcf.errors.error_500 + ":" + "Error: " + e);
	    			},
	    			503 : function(e){
	    				console.error(ohcf.errors.error_503 + ":" + "Error: " + e);
	    			}
	  			},
	  			success		: function(data){
	  				if(dataOnly === true && callback !== undefined){
	  					callback(data);
	  				}
	  				else if(dataOnly === true && callback === undefined){
	  					console.error(ohcf.errors.error_noCallback);
	  				}
	  				else if(dataOnly === false && callback !== undefined){
	  					console.error(ohcf.errors.error_dataOnly);
	  				}
	  				else{
	  					if(data.status === "success"){
			  				if(data.open === true){
			  					ohcf.defaults.openingTimesContainer.text(data.open_time + " to " + data.close_time + "(" + data.timezone + ")");
			  				}
			  				else{
			  					ohcf.defaults.openingTimesContainer.text("Closed");
			  				}
		  				}
		  				else{
		  					console.error(ohcf.errors.error_unsuccessful);
		  				}
	  				}
	  			}
			});
