	...
	omniture.setReportSuite = function(url){
		...
		try{
			...
			var suiteList = "",
				...
				vertical = ["health", "car", "life", "homeLoans", "energy", "broadband", "homeAndContents", "carMobile", "lifeMobile", "homeLoanMobile", "corporate", "multiquote"],
				...
				applicationId = setReportSuiteId,
				hostname = url.hostname;

			//Health
			if( $.inArray(hostname,prodServers) !== -1 && applicationId === vertical[0]){
				suiteList = "iselect,iselecthealthprd";
			}
			else if( $.inArray(hostname,prodServers) === -1 && applicationId === vertical[0]){
			...
			//Energy
				...
				suiteList = "iselect-dev,iselectutilitiesdev";
			}
...
var s_account = omniture.setReportSuite(window.location), s = s_gi(s_account);
...
s.linkInternalFilters="javascript:,iselect.com.au,192.168.0.142,10.153.240.192,10.153.240.57,10.153.240.181,182.23.215.194,192.168.0.55,10.153.240.187,10.153.240.196,10.153.240.198:8443,192.168.0.141:7070,192.168.0.142:8070,192.168.1.26:8080,#,tel:";
s.linkLeaveQueryString=false;
s.linkTrackVars="None";
s.linkTrackEvents="None";

/* Begin Form Analysis config */
// add fom name elements to s.formList to be tracked
s.formList="applyNowElecForm,applyNowbundleForm,applyNowGasForm";
s.trackFormList=true;
s.trackPageName=true;
s.useCommerce=true;
s.varUsed="eVar40";
// Abandon, Success, Error */
// Without a name="form_name" flag on your form tag, the abandon event will not work. */
s.eventList="event54,event55,event56";
/* End Form Analysis */
...
function s_doPlugins(s) {
  ...
  if (s.getQueryParam('ref') != null && s.getQueryParam('ref') != "") {
  s.campaign=s.getQueryParam('ref');
  } else if (s.getQueryParam('et_cid') != null && s.getQueryParam('et_cid') != "") {
  s.campaign='ET|' + s.getQueryParam('et_cid') + '|' + s.getQueryParam('et_rid') + '|' + s.getQueryParam('linkid'); /* Exact Target Query string attribute */
  }

  /* internal campaign tracking ?onsite=value */
  s.eVar25 = s.getQueryParam('onsite');
...
/* UNCHANGED, NOT COMPATIBLE WITH APPMEASUREMENT
...
/*
 * T&T Plugin: read combined cookies v0.37
 */
if(!s.__ccucr)
{
...
/*
 * T&T Plugin: write combined cookies v0.37
 */
if(!s.__ccucw)
{
